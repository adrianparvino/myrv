myrv.json: top.v furv.v alu.v decoder.v immdecoder.v rom.v uart.v ram.v firmware
	...
	yosys -x log.x -p "read_verilog immdecoder.v decoder.v alu.v furv.v rom.v ram.v uart.v top.v; proc; alumacc;; flatten;; wreduce;; opt -fine -full;; opt_merge;; share -aggressive;; synth_gowin -json myrv.json";
	# yosys -p "read_verilog immdecoder.v decoder.v alu.v furv.v rom.v ram.v uart.v top.v; synth_gowin -json myrv.json";
	# yosys -p "read_verilog immdecoder.v decoder.v alu.v furv.v rom.v ram.v uart.v top.v; proc; alumacc; opt; memory -nomap; opt; techmap; opt;; synth_gowin -json myrv.json";
...
# 	nextpnr-gowin --json $< --write $@ --device GW1NR-LV9QN88PC6/I5 --family GW1N-9C --cst tangnano9k.cst --parallel-refine --router router2
pnrmyrv.json: myrv.json tangnano9k.cst
	nextpnr-gowin --json $< --write $@ --device GW1NR-LV9QN88PC6/I5 --family GW1N-9C --cst tangnano9k.cst --parallel-refine --router router2
...
himbaechel-pnrmyrv.json: myrv.json tangnano9k.cst
	nextpnr-himbaechel --json $< --write $@ --device GW1NR-LV9QN88PC6/I5 --vopt family=GW1N-9C --vopt cst=tangnano9k.cst --router router2
